...
On montre que pour tout $n \in \N$ in existe une unique polynôme $T_n \in \R_n[X]$ tel que $T_n(\cos(\omega)) = \cos(n\omega)$. On procède pae récurrence en commençant par remarquer que que l'on a clairement $T_0(X) = 1$ et $T_1(X) = X$. Ensuite on a 
\begin{align*}
...
\[
 T_n(X) = X^n - \sum_{k=1}^m 2^{(n-1)} \widetilde{h_k^n} T_{n-2k}(X)
\]
où l'on a noté $\widetilde{h_k^n} = \begin{cases}
                                 (h_k^n)^{-1}, &\quad h_k^n \neq 0 \\
                                 0, &\quad h_k^n = 0
                                \end{cases}$.
...
On montre que $T_{n + 2}(x) + T_n(x) = 2xT_{n+1}(x)$. Pour cela on a 
\begin{align*}
